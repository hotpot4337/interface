...
export abstract class BaseAccountAPI {
	...
	async signUserOp(
		...
		const signature = this.signUserOpHash(userOpHash, proof, saltHash, nInterval, totpCode);
		return {
			...userOp,
			signature
		};
	...
	async createSignedUserOp(
		...
		const op = await this.createUnsignedUserOp(info);
		return await this.signUserOp(op, proof, saltHash, nInterval, totpCode);
	}
